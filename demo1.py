...
point_world_plot, = ax.plot([point_world[0]], [point_world[1]], [point_world[2]], marker='o', markersize=5,color='r')
for i, camera_name in enumerate(all_cameras):
        camera_name.Rt2Pose(ax) # plot camera pose
        plot_coordinate_system(ax, origin=camera_name.t, R=camera_name.R, length=1)
...
for i, axs in enumerate(axs_camviews.flat):
        ...
        plot, = axs.plot(all_cameras[i].point_camera[0], all_cameras[i].point_camera[1], marker='o', markersize=5, color=colors[i])
...
while 1:
    ...
    for i, axs in enumerate(axs_camviews.flat):
         all_cameras[i].world2camera(point_world)
         camviews_plots[i].set_data([all_cameras[i].point_frame[0]],[all_cameras[i].point_frame[1]])
